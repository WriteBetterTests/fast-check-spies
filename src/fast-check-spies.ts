...
export const getSpy = <L, A>([clear, log, fn]: [() => void, L, A]): [L, A] => {
    clear()
    return [log, fn]
}

export type SpyingArbitrary<L, A> = fc.Arbitrary<[() => void, L, A]>
  ...
  return (ma: SpyingArbitrary<L, A>) => {
    if (typeof logKeyOrF === 'function') {
      return ma.chain(([c, l, a]) => 
        ...
        .map((b: B) => [c, l, Object.assign({}, a, { [name]: b })] as any)
      );
    }
    return ma.chain(([c, l, a]) => 
